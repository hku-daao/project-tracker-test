...
Apply Supabase migrations **`028`**–**`030`**, **`032`**, **`033`**, **`036`**, **`037`**, **`038`**, **`039`**, **`045`**, and **`067`** (`urgent_reminder_sent`, `urgent_reminder_last_sent_on`, `due_today_reminder_sent_on`, `creator_due_today_reminder_sent_on`, `creator_urgent_reminder_last_sent_on`; **`036`** adds `subtask.urgent_reminder_sent` and `subtask.creator_urgent_reminder_last_sent_on`; **`037`** adds `subtask.subtask_creator_due_today_reminder_sent_on`; **`038`** adds `subtask.assignee_urgent_reminder_last_sent_on`; **`039`** adds `subtask.subtask_assignee_due_today_reminder_sent_on`; **`045`** adds `task.creator_overdue_reminder_last_sent_on`, `task.assignee_01`…`assignee_10` `_overdue_reminder_last_sent_on`, `subtask.subtask_creator_overdue_reminder_last_sent_on`, and matching `subtask.assignee_XX_overdue_reminder_last_sent_on` columns for daily overdue emails; **`067`** expands project assignees to `project.assignee_01`…`assignee_20` and stores project PICs in `project.pic_01`…`pic_20`). **Urgent (80%)** emails run only on HK calendar days **before** the due date — assignees get “You have upcoming tasks due”; sub-task **assignees** get **“You have upcoming sub-tasks due”**; task **creators** get **“An upcoming task due”** to `task.create_by`; sub-task **creators** get **“An upcoming sub-task due”** to `subtask.create_by` (skipped when `create_by` equals `assignee_01`; not sent when HK today is the sub-task due date). **Due-today** emails when HK **today** equals `due_date`: **“You have sub-tasks due today”** to sub-task assignees; **“A sub-task due today”** to `subtask.create_by`. **Overdue** emails when HK **today** is **after** `due_date` and status is not completed/deleted: **CreatorOverdueReminder** / **AssigneeOverdueReminder** (tasks) and **Subtask_CreatorOverdueReminder** / **Subtask_AssigneeOverdueReminder** (sub-tasks), each at most once per recipient per HK day until completed. **`POST /api/cron/urgent-task-reminders`** returns JSON `{ ok, urgent, assigneeUrgentSubtask, creatorUrgent, creatorUrgentSubtask, dueToday, assigneeDueTodaySubtask, creatorDueToday, creatorDueTodaySubtask, creatorOverdue, assigneeOverdue, creatorOverdueSubtask, assigneeOverdueSubtask }`. **`POST /api/cron/due-today-reminders`** returns `{ ok, dueToday, assigneeDueTodaySubtask, creatorDueToday, creatorDueTodaySubtask }`. After the due date (HK), past-due cleanup clears the earlier urgent/due-today reminder columns on tasks and subtasks (overdue columns track the last HK day an overdue email was sent).
