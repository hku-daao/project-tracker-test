...
Apply Supabase migrations **`028`**–**`030`** and **`032`** (`urgent_reminder_sent`, `urgent_reminder_last_sent_on`, `due_today_reminder_sent_on`, `creator_due_today_reminder_sent_on`). **Urgent (80%)** emails run only on HK calendar days **before** the due date; **due-today** emails run when HK **today** equals `due_date` (assignees receive “You have tasks due today”; task **creators** receive a separate **“A task due today”** email to `task.create_by`). **`POST /api/cron/urgent-task-reminders`** returns JSON `{ ok, urgent, dueToday, creatorDueToday }`. **`POST /api/cron/due-today-reminders`** returns `{ ok, dueToday, creatorDueToday }`. After the due date (HK), past-due cleanup clears the reminder columns.
