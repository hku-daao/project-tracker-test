...
Apply Supabase migrations **`028`**–**`030`**, **`032`**, **`033`**, and **`036`** (`urgent_reminder_sent`, `urgent_reminder_last_sent_on`, `due_today_reminder_sent_on`, `creator_due_today_reminder_sent_on`, `creator_urgent_reminder_last_sent_on`; **`036`** adds `subtask.urgent_reminder_sent` and `subtask.creator_urgent_reminder_last_sent_on`). **Urgent (80%)** emails run only on HK calendar days **before** the due date — assignees get “You have upcoming tasks due”; task **creators** get **“An upcoming task due”** to `task.create_by`; sub-task **creators** get **“An upcoming sub-task due”** to `subtask.create_by` (skipped when `create_by` equals `assignee_01`). **`POST /api/cron/urgent-task-reminders`** returns JSON `{ ok, urgent, creatorUrgent, creatorUrgentSubtask, dueToday, creatorDueToday }`. **`POST /api/cron/due-today-reminders`** returns `{ ok, dueToday, creatorDueToday }`. After the due date (HK), past-due cleanup clears the reminder columns on tasks and subtasks.
