...
Apply Supabase migrations **`028`**–**`030`**, **`032`**, and **`033`** (`urgent_reminder_sent`, `urgent_reminder_last_sent_on`, `due_today_reminder_sent_on`, `creator_due_today_reminder_sent_on`, `creator_urgent_reminder_last_sent_on`). **Urgent (80%)** emails run only on HK calendar days **before** the due date — assignees get “You have upcoming tasks due”; task **creators** get a separate **“An upcoming task due”** email to `task.create_by` (tracked independently via `creator_urgent_reminder_last_sent_on`). **Due-today** emails run when HK **today** equals `due_date` (assignees + creator **“A task due today”**). **`POST /api/cron/urgent-task-reminders`** returns JSON `{ ok, urgent, creatorUrgent, dueToday, creatorDueToday }`. **`POST /api/cron/due-today-reminders`** returns `{ ok, dueToday, creatorDueToday }`. After the due date (HK), past-due cleanup clears the reminder columns.
